...
def test(opt):
    ...
    if torch.cuda.is_available():
        ...
        print("place model loaded from {}".format(pl_model_path))
        pl_model = torch.nn.DataParallel(pl_model).cuda()
        pl_normalize = Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])


    # load the color map for detection results
    colors = pickle.load(open("./Yolo_v2_pytorch/src/pallete", "rb"))

    model1.eval(); pl_model.eval()
    width, height = (1024, 768)
    width_ratio = float(opt.image_size) / width
    height_ratio = float(opt.image_size) / height

    # load test clips
    temp_images=[]; temp_info=[]; temp_filename=[]
    for iter, batch in enumerate(test_loader):
        ...
        for idx in range(len(image)):
            ...
            save_place_dir = './results/place/{}/'.format(
                info_filename[idx].split('/')[4])
            if not os.path.exists(save_place_dir):
                os.makedirs(save_place_dir)
        ...
        while True:
            ...
            temp_filename += info_filename[:(10-temp_len)]
            info_filename = info_filename[(10-temp_len):]

            temp_len = len(temp_images)
            if (temp_len<10) and (iter==len(test_loader)-1):
                for pad in range(10-temp_len):
                    temp_images += temp_images[-1]
                    temp_info += temp_info[-1]
                    temp_filename += temp_filename[-1]
                temp_len = len(temp_images)
            if (temp_len == 10):
                batch_images = (torch.stack(temp_images).cuda())
                batch_images = batch_images.unsqueeze(0)

                target = torch.Tensor(temp_info).to(torch.int64).cuda()
                output = pl_model(batch_images)
                preds = torch.argmax(output, -1) # (T, n_class) ->(T, ) 
                preds = preds.tolist() 
                place_pred_list = label_remapping(preds)
                
                
                for j in range(len(preds)):
                    place_pred = place_pred_list[j]
                    f_info = temp_filename[j].split('/')
                    txt_file = "./results/place/{}/{}_{}_{}".format(
                                f_info[4], f_info[5], f_info[6], f_info[7].replace("jpg", "txt"))
                    print("txt_file:{} | place_pred:{:20}".format(
                        txt_file, place_pred))

                    f_pred = open(txt_file, mode='w')
                    pred_str = '%s\n' % (place_pred)
