...
class Face_emotion_dataset(Dataset):
        ...
        for epi in epi_list:
            ...
            for ann in ann_list:
                ...
                for idx in idx_list:
                    ...
                    for fsl in range(num_f):
                        # chosen face
                        db = self.db_dict[epi][ann][idx][fsl]
                        # load img
                        fname = 'IMAGE_'+db['img']+'.jpg'
                        fpath = os.path.join(self.db_path, epi, ann, idx, fname)
                        # face box
                        bb  = db['face_rect']
                        bb_x, bb_y, bb_w, bb_h = bb[0], bb[1], bb[2]-bb[0], bb[3]-bb[1]
                        fbb = (bb_x, bb_y, bb_w, bb_h)
                        # emo_vec
                        emo = np.array([db['emotion']])
                        # tuplet
                        self.db_list.append( (fpath, fbb, emo) )
        
        # len
        self.len = len(self.db_list)
    ...
    def __getitem__(self, i):
        # load img
        img = imread_to_rgb(self.db_list[i][0])
        # face box
        bb  = self.db_list[i][1]
